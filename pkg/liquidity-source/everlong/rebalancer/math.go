...
type VaultState struct {
	...
	// Venue gates the swap path checks on every call. SwapperRotated: the rebalancer's
	// settlementSwapper() is no longer this pool's swapper (every fill reverts
	// SettlementSwapperNotRegistered). MintDenied: the swapper fell off the ALM mint
	// allowlist (leverage reverts).
	SwapperRotated bool `json:"rot,omitempty"`
	MintDenied     bool `json:"mintDenied,omitempty"`
...
func (s *VaultState) leverageLegsActual(almRequired, maxStableIn, maxVolatileIn *big.Int) (
	physStable, physVolatile, dIdleS, dIdleV *big.Int, ok bool) {
	...
	return physStable, physVolatile, dIdleS, dIdleV, true
}
