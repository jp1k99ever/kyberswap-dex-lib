...
const (
	...
	// Per-direction gas, MEASURED on real settled fills against the deployed Berachain
	// stack (leverage 3,917,090; deleverage 2,349,802) and rounded up for headroom.
	// Leverage costs more: it flash-mints the stable leg, mints ALM shares and CollVault
	// shares, then draws debt through the CDP; deleverage retires debt and redeems.
	// Both are overridable per deployment via Config.
	defaultGasLeverage   int64 = 4_000_000
	defaultGasDeleverage int64 = 2_400_000
)
