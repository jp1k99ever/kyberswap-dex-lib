...
func TestEqualDecimalsUnscaled(t *testing.T) {
	p := entity.Pool{
		Address:  "0xpsm-0xhoney",
		Exchange: "everlong-psm",
		Type:     DexType,
		Tokens: []*entity.PoolToken{
			{Address: "0xnect", Decimals: 18, Swappable: true},
			{Address: "0xhoney", Decimals: 18, Swappable: true},
		},
		Reserves:    entity.PoolReserves{"0", "0"},
		StaticExtra: `{"psm":"0xpsm","wadOffset":1}`,
		// 5 bp both directions, matching the deployed fee law's current output.
		Extra: `{"paused":false,"entryBp":5,"exitBp":5,` +
	...
	res, err := quote(t, s, "0xhoney", "0xnect", "100000000000000000000")
	require.NoError(t, err)
	...
	res, err = quote(t, s, "0xnect", "0xhoney", "100000000000000000000")
	require.NoError(t, err)
	require.Equal(t, "99950000000000000000", res.TokenAmountOut.Amount.String())
...
func TestRedeem(t *testing.T) {
	...
	// the cap hook's single-swap ceiling clamps the burn below the book
	s = simWith(t, "0", "200000000000000000000", "200000000", "0", "0", false)
	s.Extra.MaxRedeem, _ = new(big.Int).SetString("40000000000000000000", 10)
	res, err = quote(t, s, "0xdebt", "0xusdc", "100000000000000000000")
	require.NoError(t, err)
	require.Equal(t, "40000000", res.TokenAmountOut.Amount.String())
	require.Equal(t, "60000000000000000000", res.RemainingTokenAmountIn.Amount.String())

	// the hook's per-caller outflow ceiling is a SECOND bound, in stable units
	s = simWith(t, "0", "200000000000000000000", "200000000", "0", "0", false)
	s.Extra.MaxOutflow = big.NewInt(25_000_000) // 25 USDC
	res, err = quote(t, s, "0xdebt", "0xusdc", "100000000000000000000")
	require.NoError(t, err)
	require.Equal(t, "25000000", res.TokenAmountOut.Amount.String())
}

// A direction the PSM refuses to price (feeBpFor reverts — a zero entry toll, or a rate
// at or above 100%) must not quote off a stale or assumed rate.
func TestClosedDirectionDoesNotQuote(t *testing.T) {
	...
	_, err := quote(t, s, "0xusdc", "0xdebt", "100000000")
	require.ErrorIs(t, err, ErrFeeUnavailable)

	s.Extra.ExitFeeBp = nil
	_, err = quote(t, s, "0xdebt", "0xusdc", "100000000000000000000")
	require.ErrorIs(t, err, ErrFeeUnavailable)
}
