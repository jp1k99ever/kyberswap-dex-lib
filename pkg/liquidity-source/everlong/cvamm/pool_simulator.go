...
// it; a burn derives everything from `shares`. When the snapshot carries no idle words
// the split is unknowable and the call falls back to the old scale-factor approximation,
// whose drift the coupling test bounds.
//
...
func (p *PoolSimulator) ApplyLiquidityDelta(sharesDelta, supplyBefore, used0, used1 *big.Int) (
	...
	idleKnown := p.Extra.IdleStable != nil && p.Extra.IdleVolatile != nil
	mint := sharesDelta.Sign() > 0
	if !idleKnown || (mint && (used0 == nil || used1 == nil)) {
		return p.applyScaledLiquidityDelta(supplyBefore, supplyAfter)
	}
	...
	// A deposit into a RETRACTED book re-arms it: there is no scale to multiply, so the
	// venue re-solves the coordinate and the scale from the reserves the deposit just
	// created and parks what the curve cannot express as idle (CvammALM._solveReseed).
	// Scaling zero would leave this sim retracted while the chain is quoting again.
	newX := p.Extra.XWad.ToBig()
	var kappa *big.Int
	if mint && p.Extra.Kappa.IsZero() {
		x, k, sSurplus, vSurplus := reseed(&p.Extra.Support, p.Extra.AnchorSqrtX96.ToBig(), newRes0, newRes1)
		if x == nil || k == nil || k.Sign() == 0 {
			return nil, nil, nil, nil // the venue would revert the reseed; refuse to model it
		}
		newX, kappa = x, k
		// The surplus moves reserve -> idle inside the ALM: totals do not change, the
		// priced book does.
		newRes0 = new(big.Int).Sub(newRes0, sSurplus)
		newRes1 = new(big.Int).Sub(newRes1, vSurplus)
		newIdle0 = new(big.Int).Add(newIdle0, sSurplus)
		newIdle1 = new(big.Int).Add(newIdle1, vSurplus)
		// The surplus only changes WHERE the deposit sits, so it leaves the reserve delta
		// and enters the idle one — the totals delta is unchanged.
		dRes0 = new(big.Int).Sub(dRes0, sSurplus)
		dRes1 = new(big.Int).Sub(dRes1, vSurplus)
		dIdle0 = new(big.Int).Add(dIdle0, sSurplus)
		dIdle1 = new(big.Int).Add(dIdle1, vSurplus)
		if newRes0.Sign() < 0 || newRes1.Sign() < 0 {
			return nil, nil, nil, nil
		}
	} else {
	...
	// The fee law reads the reserves and the coordinate, both of which a liquidity move
	// changes — a reseed changes them outright, and even a pro-rata leg shifts the
	// inventory weights by the rounding. Re-derive the directional fees on the new book
	// exactly as a swap does, and fall back to the conservative fold when the law cannot
	// be reproduced there.
...
// applyScaledLiquidityDelta is the pre-idle-tracking fallback: one scalar over reserves
// and kappa. It leaves idle untouched, so both domains move by the same amount.
func (p *PoolSimulator) applyScaledLiquidityDelta(supplyBefore, supplyAfter *big.Int) (
	dTotalStable, dTotalVolatile, dAccStable, dAccVolatile *big.Int) {
	scale := func(v *uint256.Int) *uint256.Int {
		u, overflow := uint256.FromBig(mulDivFloorBig(v.ToBig(), supplyAfter, supplyBefore))
		if overflow {
			return nil
		}
		return u
	...
	return dStable, dVolatile, dStable, dVolatile
}
...
// reSampleAfterBookMove re-derives both directional fees at the current book, or folds
// them to the worse of the two samples when the law cannot be reproduced — the same
// contract UpdateBalance honours after a swap.
