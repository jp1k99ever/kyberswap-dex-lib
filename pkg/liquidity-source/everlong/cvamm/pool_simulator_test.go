...
// TestApplyLiquidityDeltaReplaysTheVenue pins the exact path against CvammALM's own
// deposit/withdraw arithmetic: the legs are floored SEPARATELY over reserves and idle,
// which is the part a single pro-rata scalar over the totals cannot reproduce. A mint
// followed by a burn of the same shares must also leave the book no larger than it
// started (the venue's floors round in its favour, never the caller's).
func TestApplyLiquidityDeltaReplaysTheVenue(t *testing.T) {
	stableInCases, _ := fillableCases(t)
	require.NotEmpty(t, stableInCases)

	supply := big.NewInt(1_000_000)
	sim := simFromFixture(t, stableInCases[0], nil)
	// give the snapshot an idle slice, as the live venue has
	idle0 := new(big.Int).Div(sim.reserveStable.ToBig(), big.NewInt(97))
	idle1 := new(big.Int).Div(sim.reserveVolatile.ToBig(), big.NewInt(53))
	...
	shares := big.NewInt(250_000)
	used0 := new(big.Int).Div(new(big.Int).Mul(new(big.Int).Add(res0, idle0), shares), supply)
	used1 := new(big.Int).Div(new(big.Int).Mul(new(big.Int).Add(res1, idle1), shares), supply)

	dTotS, dTotV, dAccS, dAccV := sim.ApplyLiquidityDelta(shares, supply, used0, used1)
	require.NotNil(t, dTotS)

	// CvammALM.deposit: i = floor(idle * shares / supply) into idle, the rest into the
	// priced reserves, kappa scaled by (supply + shares) / supply.
	wantI0 := new(big.Int).Div(new(big.Int).Mul(idle0, shares), supply)
	wantI1 := new(big.Int).Div(new(big.Int).Mul(idle1, shares), supply)
	require.Zero(t, dAccS.Cmp(new(big.Int).Sub(used0, wantI0)), "stable reserve leg")
	require.Zero(t, dAccV.Cmp(new(big.Int).Sub(used1, wantI1)), "volatile reserve leg")
	require.Zero(t, dTotS.Cmp(used0), "totals move by the whole deposit")
	require.Zero(t, dTotV.Cmp(used1))
	after := new(big.Int).Add(supply, shares)
	require.Zero(t, sim.Extra.Kappa.ToBig().Cmp(new(big.Int).Div(new(big.Int).Mul(kappa0, after), supply)))
	require.Zero(t, sim.idleAccStable.ToBig().Cmp(new(big.Int).Add(idle0, wantI0)), "idle takes its pro-rata slice")

	// Burning the same shares back releases both legs floored on their own, so the book
	// cannot come out larger than it went in.
	sim.ApplyLiquidityDelta(new(big.Int).Neg(shares), after, nil, nil)
	require.LessOrEqual(t, sim.reserveStable.ToBig().Cmp(res0), 0, "a mint/burn round trip must not inflate the book")
	require.LessOrEqual(t, sim.reserveVolatile.ToBig().Cmp(res1), 0)
